  ...
  struct
    ...
    fun loadFile filename =
      let
        ...
        val compManagerHook : { manageImport : Ast.dec * EnvRef.envref -> unit
    ...
    fun pp_seq sep pr seq = intercalate sep (map pr seq)
    ...
    fun isTUPLEpat (RecordPat {def = [_], ...}) = false
      | isTUPLEpat (RecordPat {def = defs, flexibility = false}) =
        checkpat (1, defs)
      | isTUPLEpat _ = false
      ...
      let
        fun strength ty =
          ...
          | ConTy (tycon, args) =>
              (case tycon of
                 [tyc] =>
                   if Symbol.eq (Symbol.tycSymbol ("->"), tyc) then
                     0
                   else
                     2
               | _ => 2)
        ...
        fun ppTypeArgs tys =
          case tys of
            [] => ""
          | [ty] =>
              if strength ty <= 1 then
                "("
                ^ pp_typ ty indent
                ^ ")"
              else
                pp_typ ty indent
          | _ => "(" ^ pp_seq "," (fn t => pp_typ t indent) tys ^ ")"
      in
        case typ of
          VarTy t => ppTyvar t
            ...
            (case tycon of
               [tyc] =>
                 if Symbol.eq (Symbol.tycSymbol ("->"), tyc) then
                   ...
                   ^ " "
                   ^ pp_sym tyc
             | _ => ppTypeArgs args ^ " " ^ pp_symbol_list tycon)
        | RecordTy s =>
            let
              val records =
                pp_seq ","
                  (fn (sym, tv) => pp_sym sym ^ " : " ^ pp_typ tv indent)
                  s
              val f_record =
                if String.size records > 80 then
                  pp_seq ("," ^ newline (indent ^ "  "))
                    (fn (sym, tv) => pp_sym sym ^ " : " ^ pp_typ tv indent)
                    s
                else
                  records
        ...
        | TupleTy t => "(" ^ pp_seq " * " (fn p => pp_typ p indent) t ^ ")"
    ...
    fun pp_pat pat indent atom =
      let
        fun pp_pat' pat ident atom =
          ...
          | IntPat x => ExtractLiteral.int x
          | WordPat x => ExtractLiteral.word x
          | StringPat s => "\"" ^ s ^ "\""
          | CharPat c => "#" ^ "\"" ^ c ^ "\""
          | LayeredPat {varPat, expPat} =>
              lpcond atom
              ^ pp_pat' varPat indent false
              ^ " as "
              ^ pp_pat' expPat indent false
              ^ rpcond atom
          ...
          | FlatAppPat fap =>
              lpcond atom
              ^ pp_seq " "
                  (fn {item, fixity, region} => pp_pat' item indent true)
                  fap
              ^ rpcond atom
          ...
          | AppPat {constr, argument} =>
              lpcond atom
              ^ pp_pat' constr indent false
              ^ " as "
              ^ pp_pat' argument indent false
              ^ rpcond atom
          | VectorPat [] => "#[]"
          | VectorPat v =>
              "#["
      ...
      let
        fun pp_exp' exp atom indent =
          ...
          | FnExp rules =>
              ...
              ^ "fn "
              ^ (pp_seq (newline indent ^ " | ")
                   (fn r => pp_rule r env (indent ^ " "))
                   rules)
              ^ rpcond atom
          | FlatAppExp fap =>
              pp_seq " " (fn {item, fixity, region} => pp_exp' item true indent)
                fap
          | AppExp e =>
              (lpcond atom)
          ...
          | SeqExp exps =>
              let
                fun parenThunk () =
                  "("
                  ^ pp_seq (";" ^ newline indent)
                      (fn exp => pp_exp' exp false indent)
                      exps
                  ^ ")"
              ...
              in
                case exps of
                  [expr] =>
                    if (subExpCount expr) < 2 then
                      pp_exp' expr atom indent
                    else
                      parenThunk ()
                | _ => parenThunk ()
              end
          | IntExp x => ExtractLiteral.int x
          | WordExp x => ExtractLiteral.word x
          | RealExp x => ExtractLiteral.real x
          | StringExp s => "\"" ^ (String.toString s) ^ "\""
          | CharExp c => "#" ^ "\"" ^ c ^ "\""
          | (r as RecordExp fields) =>
              if isTUPLEexp r then
                "("
          ...
          | TupleExp p =>
              ...
              ^ pp_seq ", " (fn exp => pp_exp' exp false indent) p
              ^ ")"
          | SelectorExp name => lpcond atom ^ "#" ^ pp_sym name ^ rpcond atom
          ...
          let
            ...
            fun fixitypp (name, rand, leftFix, rightFix) =
              ...
              in
                case thisFix of
                  INfix _ =>
                    (case stripMark rand of
                       RecordExp [(_, pl), (_, pr)] =>
                         let
                           val atom =
                             strongerL (leftFix, thisFix) orelse strongerR
                                                                   ( thisFix
                                                                   , rightFix
                                                                   )
                           val (left, right) =
            ...
            fun appPrint (AppExp {function = rator, argument = rand}, l, r) =
                (case stripMark rator of
                 ...
                 | rator =>
                     (pp_exp' rator atom indent)
                     ^ " "
                     ^ (pp_exp' rand atom indent))
    ...
    and pp_rule (Rule {pat, exp}) env indent =
      let val patrn = pp_pat pat indent false in
        patrn
        ^ " => "
    ...
    and pp_dec dec env indent =
      let
        fun pp_dec' dec indent =
          (case dec of
             ValDec (vbs, tyvars) =>
               "val " ^ pp_seq (newline indent ^ "and ")
                          (fn v => ppVb v env indent)
                          vbs
           | ValrecDec (rvbs, tyvars) =>
               "val rec " ^ pp_seq (newline indent ^ "and ")
                              (fn r => ppRvb r env indent)
                              rvbs
           | DoDec exp => "do " ^ pp_exp exp env indent
           | FunDec (fbs, tyvars) =>
               "fun " ^ pp_seq (newline indent ^ "and ")
                          (fn f => ppFb f env indent)
                          fbs
           | TypeDec tycs => "type " ^ pp_seq " " (fn t => ppTb t indent) tycs
           | DatatypeDec {datatycs, withtycs = []} =>
               "datatype " ^ pp_seq " " (fn d => ppDb d indent) datatycs
           | DatatypeDec {datatycs, withtycs} =>
               "datatype "
           ...
           | AbstypeDec {abstycs, withtycs, body} =>
               ...
               ^ pp_dec' body indent
           | ExceptionDec ebs =>
               "exception " ^ pp_seq " " (fn p => ppEb p indent) ebs
           | StrDec sbs =>
               "structure " ^ pp_seq " " (fn s => ppStrb s env indent) sbs
           | FctDec fbs => "<functor>"
           | SigDec sigvars =>
               let
               ...
               in
                 "signature " ^ pp_seq " and " pp_sig sigvars
               end
           ...
           | SeqDec decs =>
               pp_het_seq
                 ...
                 decs ^ "\n"
           | OpenDec strbs => "open " ^ pp_seq " " pp_symbol_list strbs
           ...
           | MarkDec (dec, (s, e)) => pp_dec' dec indent)
            handle Match => (Dec.legacy {abs = (fn _ => "<abstraction?>")} dec)
      in
        pp_dec' dec indent
      end
        ...
        Vb {pat, exp, ...} =>
          ...
          in
            ...
            ^ (let
               ...
               in
                 if String.size exp_ind + size + 2 > 80 then
                   newline (indent ^ "  ") ^ (exp_p (indent ^ "  "))
                 else
                   exp_ind
               end)
          end
    ...
    and ppRvb rvb env indent =
      case rvb of
        Rvb {var, exp, ...} => pp_sym var ^ " = " ^ pp_exp exp env indent
    ...
    and ppClause cls env indent =
      case cls of
        Clause {pats, resultty, exp} =>
          let fun ppCls {item, fixity, region} = pp_pat item indent true in
            pp_seq " " ppCls pats
            ^ (case resultty of
                 SOME ty => " : " ^ pp_typ ty indent
               | NONE => "")
            ^ " = "
            ^ (let val exp = pp_exp exp env (indent ^ "  ") in
                 if String.size exp > 20 then
                   newline (indent ^ "  ") ^ exp
                 else
                   exp
               end)
    ...
    and ppTb tb indent =
      let
        fun pp_tyvar_list [] = ""
          | pp_tyvar_list symbol_list =
            let val types = pp_seq ", " ppTyvar symbol_list in
              (if List.length symbol_list >= 2 then
                 "("
                 ^ types
                 ^ ")"
               else
    ...
    and ppDbrhs rhs indent =
      let
        fun pprhs (sym, tv) =
          case tv of
            SOME a => pp_sym sym ^ " of " ^ pp_typ a indent
          | NONE => pp_sym sym
      in
    ...
    and ppEb eb indent =
      case eb of
        EbGen {exn, etype} =>
          (case etype of
             SOME a => pp_sym exn ^ " of " ^ pp_typ a indent
           | NONE => pp_sym exn)
      | EbDef {exn, edef} => pp_sym exn ^ " = " ^ pp_symbol_list edef
    ...
    and ppStrb str env indent =
      case str of
        Strb {constraint, def, name} =>
          ...
          ^ (case constraint of
               NoSig => ""
             | Transparent sigexp => " : " ^ ppSigExp sigexp env indent
             | Opaque sigexp => " :> " ^ ppSigExp sigexp env indent)
          ^ " = "
    ...
    and ppStrExp str env indent =
      ...
      | ConstrainedStr (stre, constraint) =>
          ppStrExp stre env indent ^ (case constraint of
                                        NoSig => ""
                                      | Transparent sigexp =>
      ...
      | AppStr (path, str_list) =>
          pp_symbol_list path ^ pp_seq " "
                                  (fn (strl, _) =>
                                     "("
                                     ^ ppStrExp strl env indent
                                     ^ ")")
                                  str_list
    ...
    and ppSpec spec env indent =
      let
        fun pp_tyvar_list [] = ""
          | pp_tyvar_list [ty] = ppTyvar ty
          | pp_tyvar_list tyvar_list =
            "("
            ^ pp_seq ", " ppTyvar tyvar_list
            ^ ")"
      in
        case spec of
          StrSpec sspo_list =>
            let
              fun pp (symbol, sigexp, path) =
                pp_sym symbol
                ^ " = "
                ^ ppSigExp sigexp env indent
                ^ (case path of
                     SOME p => " " ^ pp_symbol_list p
                   | NONE => "")
            in
              "structure " ^ pp_seq ", " pp sspo_list
            end
        | TycSpec (stto_list, b) =>
            let
              fun pp (symbol, tyvar_list, tyo) =
                pp_tyvar_list tyvar_list
                ^ pp_sym symbol
                ^ (case tyo of
                     SOME ty => " = " ^ pp_typ ty indent
                   | NONE => "")
            in
              "type " ^ pp_seq ("|" ^ newline (indent ^ "    ")) pp stto_list
            end
        | FctSpec sf_list =>
            let
              fun pp (symbol, fsigexp) =
                pp_sym symbol
                ^ " : "
                ^ ppFsigExp fsigexp
            in
              "functor " ^ pp_seq (newline indent ^ "and ") pp sf_list
            end
        | ValSpec st_list =>
            let
              fun pp (symbol, ty) =
                ...
                ^ pp_typ ty indent
            in
              pp_seq (newline indent ^ "and ") pp st_list
            end
        ...
        | DataSpec {datatycs, withtycs = []} =>
            "datatype " ^ pp_seq (newline indent ^ "and ")
                            (fn d => ppDb d indent)
                            datatycs
        | DataSpec {datatycs, withtycs} =>
            "datatype "
            ^ pp_seq (newline indent ^ "and ") (fn d => ppDb d indent) datatycs
            ^ newline indent
            ^ "withtype "
            ^ pp_seq (newline indent ^ "and ") (fn t => ppTb t indent) withtycs
        | ExceSpec sto_list =>
            let
              fun pp (symbol, tyo) =
                case tyo of
                  SOME ty => pp_sym symbol ^ " of " ^ pp_typ ty indent
                | NONE => pp_sym symbol
            in
              "exception " ^ pp_seq (newline indent ^ "and ") pp sto_list
            end
        | ShareStrSpec paths => "sharing " ^ pp_seq " = " pp_symbol_list paths
        | ShareTycSpec paths =>
            "sharing type " ^ pp_seq " = " pp_symbol_list paths
        | IncludeSpec sigexp => ppSigExp sigexp env indent
        | MarkSpec (m, r) => ppSpec m env indent
      end

    and ppFsigExp _ = "<functor sig>"
  end
