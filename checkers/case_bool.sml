  ...
  struct
    ...
    fun find_clause1 e p =
      case find_var_pat p of
        NotFound => NONE
      | Wildcard => NONE
      | Var "true" => if find_var_exp e "true" then SOME true else NONE
      | Var "false" => if find_var_exp e "false" then SOME false else NONE
      | Var _ => NONE

    val find_var_inv = fn e => find_var_exp e o Bool.toString o not

    fun find_clause2 e p clause1 =
      case find_var_pat p of
        NotFound => false
      | Wildcard => find_var_inv e clause1
      | Var name => (find_var_inv e clause1) orelse find_var_exp e name
    ...
    val check = find_case
    val warning =
      "(case <bool> of true => true | false => false) or some variation"
    val hint = "you don't need the case!"
  end
