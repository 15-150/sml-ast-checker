...
struct
  ...
  fun addFunctionType allFns function fb classifications =
    let
      ...
      val recursiveCalls = List.exists (fn (f, t) => is_recursive t) calledFnClassifications
      val allTailRecursiveCalls = List.all (fn (f, t) => is_tailrecursive t orelse is_nonrecursive t) calledFnClassifications
    in
